...
import { setUserHomeActon } from '../../app/feature/state'
...
const HmDivList = ({head,type}) => {
  ...
  return (
    ...
    <div className="hm-list-div-container">
        ...
        <div className="hm-list-div-link" onClick={()=>{
            dispatch(setUserHomeActon({path: location.pathname, dateTime: dateTime.toISOString(), action: `See more-${type}-btn`, desc: `clicked See more ${type} button`}))
            navigate(`/${type}`)}}>See more <BsFillArrowRightCircleFill style={{marginLeft:".5rem"}}/> </div>
    </div>:""}
    </>
    </>
  )
